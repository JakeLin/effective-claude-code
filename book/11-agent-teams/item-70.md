...
agent_steps:
  ...
  - "Re-verify flag names, modes, limitations, and config layout against current docs rather than trusting older examples"
  - "Keep agent teams out of load-bearing unattended pipelines until the API stabilizes; prefer interactive use for now"
ecc_meta:
  target: conversation
  action: suggest
  check: "CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS env var is set or agent teams are in use"
---
