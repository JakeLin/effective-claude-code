...
agent_steps:
  ...
  - "Reserve `alwaysLoad: true` for servers whose tools are called on essentially every turn, not as a blanket default"
...
ecc_meta:
  target: conversation
  action: suggest
  check: ".mcp.json or settings.json has alwaysLoad: true on multiple servers"
---
