...
ecc_meta:
  target: conversation
  action: suggest
  check: ".mcp.json exists with MCP servers that have write capabilities but .claude/settings.json has no mcp__* permission rules — skip if all servers are read-only"
---
