...
1. **Research** — two passes:
   - **Structured facts (delegate to `Explore`)**: ask the agent to read the theme's best-practice and implementation files in `references/claude-code-best-practice/` AND fetch the official docs page from `https://code.claude.com/docs`. Return under ~400 words covering frontmatter fields and defaults, official behavior, and version-specific notes. The raw doc dump belongs in the child, not the main thread (Item 15).
   - **Long-form material (read inline)**: read relevant videos, reports, and prose from `references/` directly in the main thread. This is where principles and anti-patterns surface, and delegating it flattens the texture the book is meant to capture (the limit on Item 15 — don't delegate understanding).
